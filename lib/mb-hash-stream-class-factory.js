...
// Grab the things we need from the native side. The right hand sides are the
// names of the resulting local variables.
const {
  native: {
    maxLanes,
    sizeofContext,
...
class Manager {
  ...
  _maybeComplete(context) {
    if (context) {
      ...
      // contexts.
      if (context.status[0] === hashStatus.HASH_CTX_STS_COMPLETE) {
...
Object.defineProperty(MBHashStream, 'name', { value: className });
